...
class AuthService():
    ...
    def register(self, username, password, fullname):
        u = UserLoginModel(username=username,
...
class PictureService():
    ...
    def insert(self, userlogin_id, pic):
        ...
        try:
            ...
            picture.pic = str(picture.userlogin_id)+"/" + \
                str(picture.id)+"_"+picture.pic
...
class ShareService():
    def getByUserID(self, userlogin_id):
        ...
        shares = SharePictureModel.query.filter(
            SharePictureModel.userlogin_id == userlogin_id)
        return shares

    def searchByPicID(self, picture_id):
        shares = SharePictureModel.query.filter(
            SharePictureModel.picture_id == picture_id)
        return shares
    ...
    def searchAvailableUser(self, picture_id):
        ...
        users = UserLoginModel.query.filter(
            UserLoginModel.id.notin_(list)).all()
        return users
